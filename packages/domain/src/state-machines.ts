...
export function normalizeInferenceFingerprint(text: string, itemType: string): string {
  const normalized = `${itemType.toLowerCase()}::${text.trim().toLowerCase().replace(/\s+/g, " ")}`;
  return createHash("sha256").update(normalized).digest("hex");
}

export function canCreateInference(fingerprint: string, dismissed: DismissedInference[]): boolean {
...
export function confirmInference(item: Item): Item {
...
export function editThenConfirmInference(item: Item, editedText: string): { item: Item; editHistory: ItemEditHistory } {
  if (item.state !== "INFERENCE_PENDING") {
...
export function dismissInference(item: Item, profileId: string, dismissReason: string | null): DismissedInference {
  if (item.state !== "INFERENCE_PENDING") {
...
export function expireConsentRequest(request: ConsentRequest): ConsentRequest {
...
export function decideConsentRequest(request: ConsentRequest, decision: ConsentDecisionRecord): ConsentRequest {
  if (request.state !== "PENDING") {
...
export function auditEvent(params: Omit<AuditEvent, "event_id" | "timestamp">): AuditEvent {
  return {
    ...params,
    event_id: randomUUID(),
    timestamp: nowIso()
