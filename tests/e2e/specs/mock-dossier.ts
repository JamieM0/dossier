...
export type MockDossierSeed = {
  ...
  likedMovies?: number;
  /** Pre-seeds this many synthetic Horror-genre titles already rated
   *  "not interested," and skips the pool's first 6 (non-Horror) titles
   *  so the Rate queue starts right at "Crimson Hour" (the pool's one
   *  Horror title) — lets a test reach the genre-pattern popup with a
   *  single live click instead of walking the whole queue. */
...
export function installMockDossier(seed?: MockDossierSeed): void {
  ...
  if (seed?.notInterestedHorrorSeed) {
    ...
    const horrorTemplate = pool[6]; // "Crimson Hour" — the pool's one Horror title.
    for (let i = 0; i < seed.notInterestedHorrorSeed; i++) {
      const key = `movie:${900000 + i}`;
      ratings[key] = {
        rating: -0.5,
        item: { ...horrorTemplate, id: 900000 + i, title: `Seeded Horror ${i}`, key },
