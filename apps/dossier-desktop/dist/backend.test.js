...
function createRepoMock() {
    ...
    return {
        ...
        createInference: (input) => input.text.includes("suppressed")
            ? null
            : {
                item_id: "inf-1",
        ...
        updateItem: (itemId, patch) => itemId === "missing"
            ? null
            : {
                item_id: itemId,
                state: "CONFIRMED",
        ...
        updateCompartment: (compartmentId, patch) => compartmentId === "missing" ? null : { compartment_id: compartmentId, ...asRecord(patch) },
        deleteCompartment: (compartmentId) => compartmentId !== "missing",
        setItemCompartments: (itemId, compartmentIds) => compartmentIds.map((compartmentId) => ({ item_id: itemId, compartment_id: compartmentId })),
        ...
        getConsentRequest: (requestId) => requestId === "missing"
            ? null
            : {
...
beforeEach(async () => {
    ...
    const storeService = {
        ...
        verifyLocalBackup: (backupId) => (backupId === "missing" ? null : { backupId }),
        restoreLocalBackup: (backupId) => (backupId === "missing" ? null : { backupId }),
...
describe("control API routes", () => {
    ...
    test("validates local model settings payload", async () => {
        const response = await request("/control/settings", {
            method: "PUT",
            body: JSON.stringify({
                next: {
    ...
    test("requires profile delete confirmation phrase", async () => {
        const response = await request("/control/profile/delete-irreversible", {
            method: "POST",
