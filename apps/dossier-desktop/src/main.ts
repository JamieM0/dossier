...
let storeService: DossierStoreService;
...
function setMacApplicationMenu(): void {
  if (process.platform !== "darwin") {
    return;
  }
...
function registerIpcHandlers(): void {
  ...
  ipcMain.handle("settings:set", (_event, next: Partial<DossierSettings>) => {
    settingsCache = {
      ...settingsCache,
      ...next
    };
  ...
  ipcMain.handle("data:export-encrypted", (_event, passphrase: string) => {
    return storeService.createEncryptedExport(passphrase);
  });

  ipcMain.handle("data:import-encrypted", (_event, artifact, passphrase: string) => {
    storeService.importEncryptedExport(artifact, passphrase);
  });

  ipcMain.handle("data:run-takeout-import", (_event, importPath: string) => {
...
async function bootstrap(): Promise<void> {
  ...
  await server.start();
  server.on("consent:request", (payload: unknown) => {
